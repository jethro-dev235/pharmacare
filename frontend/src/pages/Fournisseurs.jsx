...
function Fournisseurs() {
  ...
  const fetchFournisseurs = () => {
    axios.get('http://localhost:5000/api/fournisseurs', { headers }).then(res => setFournisseurs(res.data))
  }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (editId) {
      await axios.put(`http://localhost:5000/api/fournisseurs/${editId}`, form, { headers })
    } else {
      await axios.post('http://localhost:5000/api/fournisseurs', form, { headers })
    }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Supprimer ce fournisseur ?')) {
      await axios.delete(`http://localhost:5000/api/fournisseurs/${id}`, { headers })
      fetchFournisseurs()
    }
  }
  ...
  return (
    <Layout>
      <div style={styles.header}>
        <h1 style={styles.title}>🚚 Fournisseurs</h1>
        <button onClick={() => setShowForm(!showForm)} style={styles.btn}>+ Ajouter</button>
      </div>

      {showForm && (
        <div style={styles.form}>
          <h3>{editId ? 'Modifier' : 'Ajouter'} un fournisseur</h3>
      ...
      <table style={styles.table}>
        <thead>
          <tr style={styles.thead}>
            <th>Nom</th><th>Téléphone</th><th>Email</th><th>Adresse</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {fournisseurs.map(f => (
            <tr key={f.id} style={styles.row}>
              ...
              <td>{f.adresse || '-'}</td>
              <td>
                <button onClick={() => handleEdit(f)} style={styles.btnEdit}>✏️</button>
                <button onClick={() => handleDelete(f.id)} style={styles.btnDelete}>🗑️</button>
              </td>
            </tr>
          ))}
        </tbody>
